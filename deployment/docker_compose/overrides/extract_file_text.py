...
def detect_encoding(file: IO[bytes]) -> str:
    ...
    if raw_data.startswith(b"\xef\xbb\xbf"):
        return "utf-8"
    det = chardet.detect(raw_data) or {}
    enc = det.get("encoding") or "utf-8"
    conf = det.get("confidence") or 0.0
    enc_lower = enc.lower()
    if enc_lower in ("ascii", "us-ascii"):
        return "utf-8"
    legacy_blocklist = ("macroman", "iso-8859-1", "latin-1", "windows-1252", "cp1252")
    if conf >= 0.80 and enc_lower not in legacy_blocklist:
        return enc
    return "utf-8"
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            fixed_name = file_io_to_text(BytesIO(original_name))
            file_info.filename = fixed_name
            logger.info(f"DEBUG ZIP: change {original_name} to {fixed_name}")
            ...
            with zip_file.open(file_info, "r") as subfile:
