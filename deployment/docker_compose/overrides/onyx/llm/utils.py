...
_EXTRA_COMMAS_RE = re.compile(r",{3,}")
...
def _decode_unicode_escaped_string(text: str) -> str:
    if not isinstance(text, str):
        return text

    # --- CASE 1: collapse excessive commas ---
    if "," in text:
        text = _EXTRA_COMMAS_RE.sub(",,", text)
    ...
    if _UNICODE_ESCAPE_RE.search(text):
        try:
            try:
                ...
                text = text.encode("latin-1", errors="backslashreplace") \
                           .decode("unicode_escape", errors="replace")
        except Exception:
            pass

    # --- CASE 3: mojibake UTF-8 decoded as latin1 ---
...
def decode_messages_unicode(messages: list[dict]) -> list[dict]:
    def walk(value: Any) -> Any:
        if isinstance(value, str):
            return _decode_unicode_escaped_string(value)
