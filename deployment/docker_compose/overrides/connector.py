...
def decode_zip_filename(raw_name: str) -> str:
    raw_bytes = raw_name.encode("cp437", errors="replace")
    forced_encoding = os.getenv("ONYX_ZIP_FILENAME_ENCODING", "cp866").strip().lower()
    if forced_encoding:
        try:
            return raw_bytes.decode(forced_encoding, errors="replace")
        except LookupError:
            pass
            ...
            if is_zip_file(file):
                ...
                with zipfile.ZipFile(file.file, "r") as zf:
                    ...
                    for info in zf.infolist():
                        ...
                        if info.is_dir():
                            continue
                        if not should_process_file(raw_name):
                            continue
                        decoded_name = decode_zip_filename(raw_name)
                        sub_file_bytes = zf.read(info)
                        mime_type, __ = mimetypes.guess_type(decoded_name)
                        if mime_type is None:
                            mime_type = "application/octet-stream"
                        display_name = os.path.basename(decoded_name)
                        ...
                        deduped_file_names.append(display_name)
                continue
